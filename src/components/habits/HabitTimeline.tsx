        ...
        <div className="flex items-center gap-2 min-w-0 max-w-full flex-1">
          ...
          <div ref={dropdownRef} className="relative inline-block ml-1 sm:ml-1.5 min-w-0 max-w-full flex-1 sm:flex-initial">
            <button
              ...
              className="inline-flex items-center justify-between gap-1.5 sm:gap-3 px-3 sm:px-4 py-2 sm:py-2.5 w-full sm:w-auto min-w-0 max-w-full sm:min-w-[260px] rounded-2xl bg-slate-950 hover:bg-slate-900 text-white font-extrabold text-xs sm:text-sm md:text-base border border-purple-400/50 shadow-xl shadow-purple-950/60 transition-all active:scale-98"
            >
              <span className="flex items-center gap-1.5 sm:gap-2.5 min-w-0 truncate">
                <span className={`text-[9px] sm:text-xs font-black px-1.5 sm:px-2 py-0.5 rounded-lg border uppercase tracking-wider shrink-0 ${selectedDayInfo.badgeStyle}`}>
                ...
                {isViewingToday && (
                  <span className="text-[8px] sm:text-[10px] font-black px-1.5 sm:px-2 py-0.5 rounded-md uppercase tracking-wider shrink-0 bg-amber-500/20 text-amber-300 border border-amber-400/40">
                    Today
                  </span>
                )}
              </span>
              <ChevronDown className={`w-3.5 h-3.5 sm:w-4 sm:h-4 text-purple-300 shrink-0 transition-transform duration-200 ${isDropdownOpen ? 'rotate-180 text-pink-400' : ''}`} />
            </button>
