    ...
    <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-slate-950/40 p-3 sm:p-4 backdrop-blur-sm">
          ...
          {sortedPresets.map((preset) => {
            ...
            return <div key={preset.id} className={`flex items-center justify-between rounded-xl border p-3 ${active ? 'border-purple-300 bg-purple-50' : 'border-gray-200'}`}>
              ...
              <div className="flex items-center gap-1">
                ...
                <button disabled={isSubmitting} onClick={() => { setEditingId(preset.id); setEditName(preset.name); setEditColor(preset.color || '#C084FC'); }} title="Rename/Recolor period" className="rounded-lg p-1.5 text-gray-400 hover:bg-purple-50 hover:text-purple-600"><Edit2 className="h-4 w-4" /></button>
                {presets.length > 1 && <button disabled={isSubmitting} onClick={() => setDeletingPreset(preset)} title="Delete period" className="rounded-lg p-1.5 text-gray-400 hover:bg-rose-50 hover:text-rose-600"><Trash2 className="h-4 w-4" /></button>}
              </div>
            </div>;
          })}
        </div>
        ...
        <div className="p-4 sm:p-5 border-t border-purple-100 bg-purple-50/50 shrink-0">
          {isCreating ? (
            <form onSubmit={createPreset} className="space-y-3">
              ...
              <div className="flex justify-end gap-2"><button type="button" onClick={() => setIsCreating(false)} className="rounded-lg px-3 py-1.5 text-xs text-gray-600 hover:bg-gray-100">Cancel</button><button disabled={isSubmitting || !name.trim()} className="inline-flex items-center gap-1 rounded-lg bg-purple-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-purple-700 disabled:opacity-50"><Plus className="h-4 w-4" />Create Period</button></div>
            </form>
          ) : <button type="button" onClick={() => setIsCreating(true)} className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-dashed border-purple-200 py-2.5 text-xs font-bold text-purple-600 transition hover:border-purple-400 hover:bg-purple-50/50"><Plus className="h-4 w-4" />Create New Period</button>}
        </div>
      </div>
      {deletingPreset && <div className="fixed inset-0 z-[10010] flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-[2px]">
        <div className="modal-enter w-full max-w-sm space-y-4 rounded-2xl border border-rose-100 bg-white p-5 shadow-2xl">
          <div className="flex items-center gap-3"><div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-rose-100 text-rose-600"><Trash2 className="h-5 w-5" /></div><div><h3 className="font-bold text-gray-900">Delete Period?</h3><p className="text-xs text-gray-500">&quot;{deletingPreset.name}&quot; and all tasks in this period will be permanently deleted.</p></div></div>
